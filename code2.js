...
var wielkosc; // ilość wiekrszy i kolumn
var count_x = 3, count_y = 3; // ilość sektorów w pionie i poziomie
var liczba; // liczba losowana (bufor dla wypełniania)
var tabela = []; // tablica 1D

var size = 1; // odnalezienie odpowiedniej długości adresacji
...
function count()
{
	...
	wielkosc = count_x*count_y;
	// if(wielkosc === undefined)
	// {

	// }
	// count_x = Math.round(wielkosc ** (1/2));
	// count_y = Math.round(wielkosc ** (1/2));

	// for(let z = 0; count_y*count_x != wielkosc;)
	// {
	// 	if(z == 0)
	// 	{
	// 		count_y++;
...
{
	for(let z = 0; z < wielkosc; z++)
	{
		if(tabela[z][y*count_x + y1] == z1)
		{
			return false;
		}
	}
...
function chk_sqr(x, y, z1)
{
	for(let x1 = 0; x1 < count_y; x1++)
	{
		...
		{
			if(tabela[x*count_y + x1][y*count_x + y1] == z1)
			{
				return false;
			}
		}
...
{
	let tab = Array(wielkosc+1);;
	tab.fill(0);
	tab[0] = Array(wielkosc);
	tab[0].fill(0);
	...
	for(let y = 0; y < count_y; y++) 
	{
		for(let y1 = 0; y1 < count_x; y1++)
		{
			if(tabela[x*count_y + x1][y*count_x + y1] === undefined)
			{
				for(let z1 = 1; z1 <= wielkosc; z1++)
				{
			...
			if(tab[0][y * count_x + y1] == 1)
			{
				...
				tabela[x*count_y + x1][y*count_x + y1] = z1;
				z++;
				y = 0;
				y1 = -1;
				tab.fill(0);
	...
	let z1 = Math.round(Math.random()*(wielkosc-1) + 1);
...
function gener()
{
	...
	for (let z = wielkosc - 1; z >= 10; size++) 
	{
		z = z / 10;
	}
	...
	for(let xyz = 0; xyz < count_y; xyz++)
	{
		...
		{
			for(let xyz3 = 1; xyz3 <= wielkosc; xyz3++)
			{
	...
	for(let x =  0; x < count_x; x++)
	{
		for(let y = 0; y < count_y; y++)
		{
			for(let z = 0; z < wielkosc/3 + parseFloat(document.querySelector("#difficulty").value); z++)
			{
				let y1 = Math.round(Math.random() * (count_x-1));
				let x1 = Math.round(Math.random() * (count_y-1));
				tabela[x*count_y + x1][y*count_x + y1] = undefined;
			}
		}
	}
			...
			for(let x1 = 0; x1 < count_y; x1++) 
			{
				...
				for(let y1 = 0; y1 < count_x; y1++) 
				{
					...
					else
					{
						...
						td.innerHTML = tabela[x*count_y + x1][y*count_x + y1];
					}
				}
			}
		}
